...
const types = [{ name: `side`, values: [`start`, `end`] }, { name: `standing`, values: [`above`, `right`, `below`, `left`] }, { name: `flow`, values: [`column`, `row`] }];
...
const centerOfBounds = (flow, axis, bounds) => bounds[axes[flow][axis].start] + bounds[axes[flow][axis].size] / 2;

const centerOfBoundsFromBounds = (flow, axis, boundsTo, boundsFrom) => centerOfBounds(flow, axis, boundsTo) - boundsFrom[axes[flow][axis].start];
...
El.calcSize = el => el === window ? { w: el.innerWidth, h: el.innerHeight } : { w: el.offsetWidth, h: el.offsetHeight };

El.calcScrollSize = el => el === window ? {
...
const getPreferenceType = preference => types.reduce((found, type) => found ? found : type.values.indexOf(preference) !== -1 ? type.name : null, null);

/* Dimension Fit Checks */

const fitWithinChecker = dimension => (domainSize, itemSize) => domainSize[dimension] >= itemSize[dimension];

const doesWidthFitWithin = fitWithinChecker(`w`);
const doesHeightFitWithin = fitWithinChecker(`h`);

const doesFitWithin = (domainSize, itemSize) => doesWidthFitWithin(domainSize, itemSize) && doesHeightFitWithin(domainSize, itemSize);

/* Errors */

const createPreferenceError = givenValue => new Error(`The given layout placement of "${ givenValue }" is not a valid choice. Valid choices are: ${ validTypeValues.join(` | `) }.`);
...
const pickZone = (opts, frameBounds, targetBounds, size) => {
  ...
  const f = frameBounds;
  const zones = [{ side: `start`, standing: `above`, flow: `column`, order: -1, w: f.x2, h: t.y }, { side: `end`, standing: `right`, flow: `row`, order: 1, w: f.x2 - t.x2, h: f.y2 }, { side: `end`, standing: `below`, flow: `column`, order: 1, w: f.x2, h: f.y2 - t.y2 }, { side: `start`, standing: `left`, flow: `row`, order: -1, w: t.x, h: f.y2 }];
  ...
  */
  ...
  if (opts.preferPlace) {
    ...
    const preferredAvailZones = availZones.filter(zone => zone[preferenceType] === opts.preferPlace);
    ...
    const preferredZones = zones.filter(zone => zone[preferenceType] === opts.preferPlace);
...
export { El, types, validTypeValues, calcRelPos, place, pickZone, axes, centerOfSize, centerOfBounds, centerOfBoundsFromBounds, doesFitWithin, equalRecords as equalCoords };